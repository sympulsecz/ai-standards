...
Model Context Protocol (MCP) is an open standard that lets you extend AI coding tools like Claude Code and Cursor with custom capabilities—connecting them to your databases, APIs, internal tools, and data sources.
...
!!! warning "Data Exposure"
    When AI tools use MCP servers, data from those servers goes into the AI's context and may be sent to AI providers. Use MCP servers only with data appropriate for AI processing. See [Data Handling](../security/data-handling.md) for classification guidance.
...
**Clear descriptions**: AI chooses when to use tools based on descriptions. Be specific about what each tool does and when to use it.

**Input validation**: Always validate tool inputs—check types, ranges, permissions, and guard against injection attacks.

**Useful errors**: Return actionable error messages. "Permission denied accessing /internal/data" is better than "Error 403".

**Appropriate scope**: Each server should have a focused purpose. Separate unrelated capabilities into different servers.
...
**Start small**: Begin with one high-value server (e.g., documentation access) and expand based on usage.

**Monitor usage**: Track which servers and tools get used most to prioritize improvements.

**Version your servers**: As you update server capabilities, maintain compatibility or clearly communicate breaking changes.

**Document for AI and humans**: Server descriptions help AI, but also document for team members configuring tools.

**Test with actual tools**: Don't just test servers in isolation—verify they work well in actual AI tool workflows.
...
- [Agents: Understanding how AI tools work](../agents/index.md) - MCP servers provide tools that agents use
- [Security: Data Handling](../security/data-handling.md) - Classify data before exposing via MCP
- [Official MCP Documentation](https://modelcontextprotocol.io/) - Implementation details and SDK references
